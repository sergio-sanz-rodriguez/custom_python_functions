...
# 1. Create a ViT class that inherits from nn.Module using already implemented transformer classes from torchvision
class ViTv2(nn.Module):
    ...
    def __init__(self,
        ...
        # 9. Create a single transformer encoder layer
        ...
        # 10. Crete the stacked transformer encoder
    ...
    def forward(self, x):
        ...
        # 14. Create class token embedding and expand it to match the batch size (equation 1)
        class_token = self.class_embedding.expand(batch_size, -1, -1) # "-1" means to infer the dimension (try this line on its own)

        # 15. Create patch embedding (equation 1)
        x = self.patch_embedding(x)

        # 16. Concat class embedding and patch embedding (equation 1)
        x = torch.cat((class_token, x), dim=1)

        # 17. Add position embedding to patch embedding (equation 1)
        #x = self.position_embedding + x
        x = x + self.position_embedding[:, :x.size(1), :]


        # 18. Run embedding dropout (Appendix B.1)
        x = self.embedding_dropout(x)

        # 19. Pass patch, position and class embedding through transformer encoder layers (equations 2 & 3)
        x = self.transformer_encoder(x)

        # 20. Put 0 index logit through classifier (equation 4)
        x = self.classifier(x[:, 0]) # run on each sample in a batch at 0 index
